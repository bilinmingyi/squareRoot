...
var printRendering = function (type, dataObj, vue, fn) {
  ...
  this.render = function () {
    ...
    this.template.pageContent.header.forEach(function (line) {
      resultStr += ('<div class="displayFlex align-items-flex-start" style="' + borderMap[line.border] + 'min-height: 5px;">')
  ...
  Promise.all([
    ...
    if (respone[0].data.code == 1000 && respone[1].data.code == 1000 && respone[2].data.code == 1000) {
      ...
      var commonVar = {
        ...
        '$处方订单号': new Date(order.create_time).format('yyyyMMdd') + order.order_seqno.slice(-6),
      ...
      switch (recipe.recipe_type) {
        case 1:
          printObject = new printRendering(
            'herbalRecipe',
              ...
              if (therapyIndex !== undefined && therapyIndex !== null) {
                printObject = new printRendering(
                  ...
                  Object.assign({}, commonVar, {
                    ...
                    "$处方金额": (recipe.therapy_list[therapyIndex].price * recipe.therapy_list[therapyIndex].num).toFixed(2)
                  }),
                  vue,
                  fn
                )
...
function printReport(vue, order, fn) {
  ...
  Promise.all([
    ...
    if (respone[0].data.code == 1000 && respone[1].data.code == 1000 && respone[2].data.code == 1000) {
      ...
      printObject = new printRendering('patientHistory', commonVar, vue, fn);
    } else {
      modal('打印失败1，请重试！')
    }
  }).catch(function (error) {
