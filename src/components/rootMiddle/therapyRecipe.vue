          ...
          <tr v-for="(item,index) in currentData.data.items">
            ...
            <template v-if="item.is_match===1">
              ...
              <td v-if="currentData.data.type == 3" style="width: 150px">
                <Input type="text" :value="item.position" @on-change="modify_medicine({key:'position',val:$event.target.value,index:index})" />
              </td>
              <td>{{item.price|priceFormat}}</td>
              <td>
                <Input type="text" :value="item.remark" @on-change="modify_medicine({key:'remark',val:$event.target.value,index:index})" />
              </td>
            </template>
            <template v-else>
