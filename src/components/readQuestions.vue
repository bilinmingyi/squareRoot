  ...
  <div class="prescription_tab_bg">
    <div class="prescription_tab">
      <div class="alert-title">
        <div class="outpatient-name">{{answer.name}}</div>
          ...
          <section v-if="item.type === 'radio'">
            ...
            <div class="mt16 mb16 align-item">
              <RadioGroup v-model="item.answer">
                <Radio v-for="opt in item.options" :label="opt" class="mr30" :key="item.id + opt" disabled></Radio>
                <Radio v-if="item.has_comment == 1" label="其他" disabled></Radio>
                <div v-if="item.has_comment == 1" style="display: inline-block">
                  (<input class="other-input" v-model="item.comment" readonly/>)
                </div>
              </RadioGroup>
            </div>
          </section>
          <section v-if="item.type === 'checkbox'">
            ...
            <div class="mt16 mb16 align-item">
              <CheckboxGroup v-model="item.answer">
                <Checkbox v-for="opt in item.options" :label="opt" class="mr30" :key="item.id + opt" disabled></Checkbox>
                <Checkbox v-if="item.has_comment == 1" label="其他" disabled></Checkbox>
                <div v-if="item.has_comment == 1" style="display: inline-block">
                  (<input class="other-input" v-model="item.comment" readonly/>)
                </div>
              </CheckboxGroup>
          ...
          <section v-if="item.type === 'input'">
            ...
            <div class="mt16 mb16 align-item" style="width: 30.25rem">
              <Input :autosize="{minRows: 2,maxRows: 5}" type="textarea" placeholder="请填写回答"
                     v-model="item.answer" disabled></Input>
            </div>
          </section>
        </div>
      </div>
...
import {readQuestions} from "@/fetch/api.js";
...
export default {
  ...
  methods: {
    getData () {
      ...
      readQuestions({
        'id': this.answer.patient_answer_id
      }).then(res => {
